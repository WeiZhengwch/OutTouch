...
import static net.minecraft.client.settings.GameSettings.Options.CLIENT_COMBAT_REACH;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && mc.theWorld != null) {
            ...
            if (mc.playerController.extendedReach()) {
                d0 = 6.0D;
                d1 = 6.0D;
            } else if (d0 > mc.gameSettings.getOptionFloatValue(CLIENT_COMBAT_REACH)) {
                flag = true;
            }
            ...
            if (mc.gameSettings.getOptionFloatValue(CLIENT_COMBAT_REACH) > 3.0F) {

                final MovingObjectPosition movingObjectPosition = entity.rayTrace(d1, partialTicks);

                if (movingObjectPosition != null) {
                    d0 = mc.gameSettings.getOptionFloatValue(CLIENT_COMBAT_REACH);
                }
            }
            ...
            if (pointedEntity != null && vec3.distanceTo(vec33) > mc.gameSettings.getOptionFloatValue(CLIENT_COMBAT_REACH)) {
                pointedEntity = null;
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (mc.getRenderViewEntity() instanceof EntityLivingBase entitylivingbase && false) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    MovingObjectPosition f6 = mc.theWorld.rayTraceBlocks(new Vec3(d0 + (double) (f3 *= 0.1f), d1 + (double) (f4 *= 0.1f), d2 + (double) (f5 *= 0.1f)), new Vec3(d0 - d4 + (double) f3 + (double) f5, d1 - d6 + (double) f4, d2 - d5 + (double) f5));
                    if (f6 == null) continue;
                    double f7 = f6.hitVec.distanceTo(new Vec3(d0, d1, d2));
                    if (!(f7 < d3) || mc.gameSettings.viewclip) continue;
                    d3 = f7;
                }
...
//        clippinghelper.disabled = Config.isShaders() && !Shaders.isFrustumCulling();
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        GL11.glFog(GL11.GL_FOG_COLOR, setFogColorBuffer(fogColorRed, fogColorGreen, fogColorBlue));
        GL11.glNormal3f(0.0F, -1.0F, 0.0F);
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(mc.theWorld, entity, partialTicks);
        float f = -1.0F;
    ...
    private void frameInit() {
        ...
        if (mc.currentScreen instanceof MainMenu) {
            updateMainMenu((MainMenu) mc.currentScreen);
        }
