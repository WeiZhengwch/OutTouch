...
public class ItemRenderer {
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (itemToRender != null) {
                if (itemToRender.getItem() instanceof ItemMap) {
                    ...
                    switch (enumaction) {
                        case NONE -> transformFirstPersonItem(f, 0.0F);
                        case EAT, DRINK -> {
                        ...
                        case BLOCK -> {
                            ...
                            GlStateManager.translate(-0.385F, -0.225F, 0.05F);
                        }
                        case BOW -> {
                            transformFirstPersonItem(f, f1);
    ...
    private void renderFireInFirstPerson() {
        if (!mc.thePlayer.isPotionActive(Potion.fireResistance.id)) {
            Tessellator tessellator = Tessellator.getInstance();
            WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        ...
        if (itemToRender != null && itemstack != null) {
            if (!itemToRender.getIsItemStackEqual(itemstack)) {
                if (Reflector.ForgeItem_shouldCauseReequipAnimation.exists()) {
                    boolean flag1 = Reflector.callBoolean(itemToRender.getItem(), Reflector.ForgeItem_shouldCauseReequipAnimation, itemToRender, itemstack, equippedItemSlot != entityplayer.inventory.currentItem);
