...
public abstract class Render<T extends Entity> implements IEntityRenderer {
    ...
    protected void renderName(T entity, double x, double y, double z) {
        if (canRenderName(entity)) {
            renderLivingLabel(entity, entity.getDisplayName().getFormattedText(), x, y, z, false);
        }
    }
    ...
    protected void renderOffsetLivingLabel(T entityIn, double x, double y, double z, String str, float p_177069_9_, double p_177069_10_) {
        renderLivingLabel(entityIn, str, x, y, z, false);
    }
    ...
    public void doRenderShadowAndFire(Entity entityIn, double x, double y, double z, float yaw, float partialTicks) {
        if (renderManager.options != null) {
            ...
            if (entityIn.canRenderOnFire() && (!(entityIn instanceof EntityPlayer) || !((EntityPlayer) entityIn).isSpectator())) {
                renderEntityOnFire(entityIn, x, y, z);
            }
        }
    }
     ...
     */
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, boolean canRendernametag) {
        ...
        if (d0 <= (double) (64 * 64)) {
            ...
            if (mc.gameSettings.nametag && entityIn instanceof EntityPlayer && entityIn != mc.thePlayer && !canRendernametag) {
                EntityPlayer ent;
                int health = (int) ((EntityPlayer) entityIn).getHealth();
                int maxh = (int) ((EntityPlayer) entityIn).getMaxHealth();
                for (Entity entity : mc.theWorld.getLoadedEntityList()) {
                    if (!(entity instanceof EntityPlayer) || (ent = (EntityPlayer) entity) == mc.thePlayer) continue;
                ...
                fontrenderer.drawString(String.format("%.1f", ((EntityPlayer) entityIn).getHealth()), (int) entityIn.height, i - 10, 65280);
                fontrenderer.drawString(String.format("%.0f", Math.sqrt(mc.thePlayer.getDistanceSqToEntity(entityIn))), (int) entityIn.height - 20, i - 10, 16776960);
