    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            if (Reflector.ForgeEntity_shouldRiderSit.exists()) {
                mainModel.isRiding = entity.isRiding() && entity.ridingEntity != null && Reflector.callBoolean(entity.ridingEntity, Reflector.ForgeEntity_shouldRiderSit);
            }
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (canRenderName(entity)) {
                ...
                f = mc.gameSettings.nametag ? 64 : (entity.isSneaking() ? NAME_TAG_RANGE_SNEAK : NAME_TAG_RANGE);
                ...
                if (d0 < (double) (f * f)) {
                    ...
                    if (entity.isSneaking()) {
                        ...
                        worldrenderer.pos(i + 1, -1.0D, 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).endVertex();
                        tessellator.draw();
                        if (mc.gameSettings.nametag && entity instanceof EntityPlayer && entity != mc.thePlayer) {
                            EntityPlayer ent;
                            int health = (int) ((EntityPlayer) entity).getHealth();
                            int maxh = (int) ((EntityPlayer) entity).getMaxHealth();
                            for (Entity entity1 : mc.theWorld.getLoadedEntityList()) {
                                if (!(entity1 instanceof EntityPlayer) || (ent = (EntityPlayer) entity1) == mc.thePlayer)
                                    continue;
                                Scoreboard scoreboard = ((EntityPlayer) entity1).getWorldScoreboard();
                                ScoreObjective scoreobjective = scoreboard.getObjectiveInDisplaySlot(1);
                                if (ent.isInvisible() && scoreobjective != null && scoreobjective.getDisplayName().substring(4).equals("SKYWARS")) {
                                    mc.theWorld.removeEntity(ent);
                                }
                            }
                            worldrenderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
                            worldrenderer.pos(-25, 92 - (92 / maxh * health), 0.0D).color(0.0F, 0.75F, 0.0F, 1.0F).endVertex();
                            worldrenderer.pos(-25, 92, 0.0D).color(0.0F, 0.75F, 0.0F, 1.0F).endVertex();
                            worldrenderer.pos(-22, 92, 0.0D).color(0.0F, 0.75F, 0.0F, 1.0F).endVertex();
                            worldrenderer.pos(-22, 92 - (92 / maxh * health), 0.0D).color(0.0F, 0.75F, 0.0F, 1.0F).endVertex();
                            tessellator.draw();
                            GlStateManager.enableTexture2D();
                            fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2, 0, -1);
                            fontrenderer.drawString(String.format("%.1f", Minecraft.getMinecraft().thePlayer.getHealth()), (int) entity.height, -10, 65280);
                            fontrenderer.drawString(String.format("%.0f", Math.sqrt(mc.thePlayer.getDistanceSqToEntity(entity))), (int) entity.height - 20, -10, 16776960);
    ...
    protected boolean canRenderName(T entity) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (team != null) {
                ...
                if (Minecraft.getMinecraft().gameSettings.nametag && !entity.getDisplayName().getFormattedText().substring(2).startsWith("[NPC]")) {
                    return true;
                }
            ...
            if (Minecraft.getMinecraft().gameSettings.nametag) {
