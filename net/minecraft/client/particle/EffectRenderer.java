...
public class EffectRenderer {
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 2; ++j) {
                ...
                if (!fxLayers[i][j].isEmpty()) {
                    ...
                    switch (i) {
                        case 0 -> renderer.bindTexture(particleTextures);
                        case 1 -> renderer.bindTexture(TextureMap.locationBlocksTexture);
                        default -> renderer.bindTexture(particleTextures);
                    }
    ...
    public void addBlockHitEffects(BlockPos p_addBlockHitEffects_1_, MovingObjectPosition p_addBlockHitEffects_2_) {
        ...
        if (iblockstate != null) {
            boolean flag = Reflector.callBoolean(iblockstate.getBlock(), Reflector.ForgeBlock_addHitEffects, worldObj, p_addBlockHitEffects_2_, this);
