...
public class Cartesian {
    public static <T> Product<? super Object> cartesianProduct(Class<T> clazz, Iterable<? extends Iterable<? extends T>> sets) {
    ...
    static class Product<T> implements Iterable<T[]> {
        ...
        public Iterator iterator() {
            return (iterables.length <= 0 ? Collections.singletonList((Object[]) createArray(clazz, 0)).iterator() : new Cartesian.Product.ProductIterator(clazz, iterables));
        }
            ...
            public T[] next() {
                if (!hasNext()) {
                    ...
                    while (index < iterators.length) {
                        results[index] = (T) iterators[index].next();
                        ++index;
                    }
