...
public class Block {
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos pos, EnumFacing side) {
        Block block = worldIn.getBlockState(pos).getBlock();
        switch (side) {
            case NORTH, SOUTH, EAST, WEST, DOWN -> {
                if ((block == Blocks.snow_layer && worldIn.getBlockState(pos).<Integer>getValue(BlockSnow.LAYERS).intValue() == 8) || (block == Blocks.lava && worldIn.getBlockState(pos.up()).getBlock() == Blocks.lava)) {
                    return false;
                }
                return !block.isOpaqueCube() && block != this;
            }
            case UP -> {
                if ((getSlabsList(worldIn, pos) && worldIn.getBlockState(pos).getValue(BlockSlab.HALF) == BlockSlab.EnumBlockHalf.BOTTOM) || (getStairsList(worldIn, pos) && worldIn.getBlockState(pos).getValue(BlockStairs.HALF) == BlockStairs.EnumHalf.BOTTOM) || (block == Blocks.lava || block == Blocks.flowing_lava)) {
                    return false;
                }
                return !block.isOpaqueCube() && block != this && block != Blocks.snow_layer && block != Blocks.daylight_detector_inverted && block != Blocks.daylight_detector && block != Blocks.unpowered_repeater && block != Blocks.unpowered_comparator && block != Blocks.powered_repeater && block != Blocks.powered_comparator && block != Blocks.enchanting_table && block != Blocks.end_portal_frame && block != Blocks.carpet && block != Blocks.farmland;
            }
        }
        return !block.isOpaqueCube();
    }

    public BlockPos sideOppositePos(BlockPos pos, EnumFacing side) {
        return switch (side) {
    ...
    public EnumFacing oppositeFacing(EnumFacing side) {
        switch (side) {
            case EAST -> {
    ...
    public boolean getStairsList(IBlockAccess worldIn, BlockPos pos) {
        Block block = worldIn.getBlockState(pos).getBlock();
        return block == Blocks.stone_stairs || block == Blocks.sandstone_stairs || block == Blocks.oak_stairs || block == Blocks.stone_brick_stairs || block == Blocks.spruce_stairs || block == Blocks.nether_brick_stairs || block == Blocks.birch_stairs || block == Blocks.jungle_stairs || block == Blocks.acacia_stairs || block == Blocks.dark_oak_stairs || block == Blocks.quartz_stairs || block == Blocks.red_sandstone_stairs || block == Blocks.brick_stairs;
    }

    public boolean getSlabsList(IBlockAccess worldIn, BlockPos pos) {
        Block block = worldIn.getBlockState(pos).getBlock();
